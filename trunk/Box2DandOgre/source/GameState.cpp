...
{
	GameFramework::getSingletonPtr()->log_->logMessage("Entering GameState...");
	
	m_pSceneMgr = GameFramework::getSingletonPtr()->root_->createSceneManager(ST_GENERIC, "GameSceneMgr");
	...
	GameFramework::getSingletonPtr()->GUIRenderer_->setTargetSceneManager(m_pSceneMgr);

	/*
	GameFramework::getSingletonPtr()->GUIRenderer_->setDefaultMouseCursor((CEGUI::utf8*)"TaharezLook", (CEGUI::utf8*)"MouseArrow"); 
    CEGUI::MouseCursor::getSingleton().setImage("TaharezLook", "MouseArrow"); 
	const OIS::MouseState state = GameFramework::getSingletonPtr()->mouse_->getMouseState();
	...
	GameFramework::getSingletonPtr()->GUIRenderer_->setGUISheet(m_pMainWnd);
...
{
	GameFramework::getSingletonPtr()->log_->logMessage("Pausing GameState...");

	GameFramework::getSingletonPtr()->GUIRenderer_->setGUISheet(0);
	GameFramework::getSingletonPtr()->GUIRenderer_->setTargetSceneManager(0);
...
{
	GameFramework::getSingletonPtr()->log_->logMessage("Resuming GameState...");

	GameFramework::getSingletonPtr()->viewport_->setCamera(m_pCamera);
	GameFramework::getSingletonPtr()->GUIRenderer_->setTargetSceneManager(m_pSceneMgr);

	GameFramework::getSingletonPtr()->GUIRenderer_->setGUISheet(CEGUI::WindowManager::getSingleton().getWindow("AOF_GUI_GAME"));
...
void GameState::exit()
{
	GameFramework::getSingletonPtr()->log_->logMessage("Leaving GameState...");
...
bool GameState::keyPressed(const OIS::KeyEvent &keyEventRef)
{
	if(m_bChatMode == true)
	{
		GameFramework::getSingletonPtr()->GUIRenderer_->injectKeyDown(keyEventRef.key);
		GameFramework::getSingletonPtr()->GUIRenderer_->injectChar(keyEventRef.text);
	}
	...
	if(GameFramework::getSingletonPtr()->keyboard_->isKeyDown(OIS::KC_TAB))
	{
	...
	if(m_bChatMode && GameFramework::getSingletonPtr()->keyboard_->isKeyDown(OIS::KC_RETURN) ||
		GameFramework::getSingletonPtr()->keyboard_->isKeyDown(OIS::KC_NUMPADENTER))
	{
...
{
	GameFramework::getSingletonPtr()->GUIRenderer_->injectMouseWheelChange(evt.state.Z.rel);
	GameFramework::getSingletonPtr()->GUIRenderer_->injectMouseMove(evt.state.X.rel, evt.state.Y.rel);
...
bool GameState::mousePressed(const OIS::MouseEvent &evt, OIS::MouseButtonID id)
{
	if(id == OIS::MB_Left)
    {
		...
		GameFramework::getSingletonPtr()->GUIRenderer_->injectMouseButtonDown(CEGUI::LeftButton);
    } 
...
bool GameState::mouseReleased(const OIS::MouseEvent &evt, OIS::MouseButtonID id)
{
	if(id == OIS::MB_Left)
    {
		...
		GameFramework::getSingletonPtr()->GUIRenderer_->injectMouseButtonUp(CEGUI::LeftButton);
    } 
    else if(id == OIS::MB_Right)
    {
        m_bRMouseDown = false;
...
void GameState::onLeftPressed(const OIS::MouseEvent &evt)
{
    ...
    for(itr = result.begin(); itr != result.end(); itr++)
    {
        if(itr->movable)
        {
			GameFramework::getSingletonPtr()->log_->logMessage("MovableName: " + itr->movable->getName());
			m_pCurrentObject = m_pSceneMgr->getEntity(itr->movable->getName())->getParentSceneNode();
			GameFramework::getSingletonPtr()->log_->logMessage("ObjName " + m_pCurrentObject->getName());
